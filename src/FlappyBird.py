...
class FlappyBird():
    ...
    def __init__(self) -> None:
        ...
        self.background = pygame.image.load('img/background-black.png') #sky
        ...
        self.flappyBird = self.Bird(50, int(self.SCREEN_HEIGHT / 2))
    ...
    def play(self, action):
        ...
        if self.gameOver == False and self.flying == True:
            # Generate pipe
            if(len(self.pipeGroup) == 0):
                pipeHeight = random.randint(0, 23)
                bottomPipe = self.Pipe(self.SCREEN_WIDTH - 23, int(self.SCREEN_HEIGHT / 2) + pipeHeight, -1)
                topPipe = self.Pipe(self.SCREEN_WIDTH - 23, int(self.SCREEN_HEIGHT / 2) + pipeHeight, 1)
